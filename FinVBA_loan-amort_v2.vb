Sub Loan_Amort_V3()
    ...
    'this program creates a year-by-year repayment schedule for a fixed rate 'loan that is to be repayed in equal annual installment payable at the end of 'each year. determine the required annual payment amount by iteration instead of using VBA's pmt funcion. the worksheet must already have proper headings and labels; the program does not create them or do their formatting.

    Dim intRate As Single, initLoanAmnt As Single, loanLife As Single
    Dim yrBegBal As Single, yrEndBal As Single
    Dim annualPmnt As Single, annualIncr as single,numOfIterations as Integer
    dim balTolerance as Single
...
' set annual payment upfront with the vale of (initLoanAmnt/loanLife)
    ...
    'annualIncr = yrEndBal / loanLife / 2 'step size for payment guess
    annualIncr = yrEndBal * (1 + intRate)^(-loanLife) / loanLife 'step size for payment guess
    annualPmnt = annualPmnt + annualIncr
    numOfIterations = numOfIterations + 1
loop while yrEndBal >= balTolerance
